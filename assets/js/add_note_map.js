var addNoteMap = {
	...
	initialize: function () {
		...
		this.map = new google.maps.Map(document.getElementById("add_note_map"), myOptions);

		google.maps.event.addListener(this.map, 'click', function (event) {//点击时出现的提示窗口，这里显示经纬度
			addNoteMap.delMarker();
			var Latitude = event.latLng.lat().toFixed(5);
			var longitude = event.latLng.lng().toFixed(5);
			addNoteMap.addMarker(event.latLng, "name", "<b>Location</b><br>" + Latitude + "," + longitude,
				Latitude + "," + longitude);
		});
	...
	addMarker: function (Gpoint, name, contentString, geo) {//添加地图上的标记
		...
		google.maps.event.addListener(marker, 'click', function () {//添加标记
			addNoteMap.infowindow.setPosition(this.position);
			addNoteMap.infowindow.setContent(contentString);
			addNoteMap.infowindow.open(addNoteMap.map, marker);
		});

		addNoteMap.map.panTo(Gpoint);
