...
<head>
	...
	<link rel="stylesheet" href="<?= base_url("assets/css/sign-up-login.css") ?>">
	<link rel="stylesheet" type="text/css" href="<?= base_url("assets/css/font-awesome.min.css") ?>">
	<link rel="stylesheet" href="<?= base_url("assets/css/inputEffect.css") ?>"/>
	<link rel="stylesheet" href="<?= base_url("assets/css/tooltips.css") ?>"/>
	<link rel="stylesheet" href="<?= base_url("assets/css/spop.min.css") ?>"/>

	<script src="<?= base_url("assets/js/jquery.min.js") ?>"></script>
<!--	<script src="--><?//= base_url("assets/js/snow.js") ?><!--"></script>-->
	<script src="<?= base_url("assets/js/jquery.pure.tooltips.js") ?>"></script>
	<script src="<?= base_url("assets/js/spop.min.js") ?>"></script>
	<script>
		(function () {
		...
		function login() {//登录
			...
			if (username == "") {
				$.pt({
					...
					width: 'auto',
					height: 'auto',
					content: "Account can't be empty"
				});
				flag = true;
			}
			if (password == "") {
			...
			if (!regExp.test(username)) {
				$.pt({
					...
					width: 'auto',
					height: 'auto',
					content: "Account must be alphabet or number under 15 characters."
				});
				flag = true;
			}
			...
			if (flag) {
				...
				//调用后台登录验证的方法
				var url1 = "<?= base_url("index.php/Welcome/login") ?>";
					...
					success: function (data) {
						if (data == true)
							...
							$.pt({
								...
								width: 'auto',
								height: 'auto',
								content: "Account or password error!"
							});
					} //成功执行方法
				});
				return false;
		...
		function register() {
			...
			if (password == "") {
				...
				if (password != repassword) {
					$.pt({
						...
						width: 'auto',
						height: 'auto',
						content: "Two different password"
					});
					flag = true;
				}
			}
			...
			if (flag) {
				...
				var url1 = "<?= base_url('index.php/Welcome/register') ?>";
				var form = new FormData($("#register_form")[0]);
				$.ajax({
					url: url1,
					type: 'POST',
					data: form,
					...
					success: function (data) {
						if (data == "ok") {
							spop({
								template: '<h4 class="spop-title">Register Success</h4>Back to login in 3 seconds',
								position: 'top-center',
								style: 'success',
								autoclose: 3000,
								onOpen: function () {
									...
									var showPop = setInterval(function () {
										...
										$('.spop-body').html('<h4 class="spop-title">Register Success</h4>Back to login in ' + second + ' seconds');
										second--;
									}, 1000);
								},
								onClose: function () {
						...
						} else if (data == "duplicate_account") {
							$.pt({
								target: $("#register-username"),
								position: 'r',
								align: 't',
						...
						} else if (data == "different_password") {
							$.pt({
								target: $("#register-password"),
								position: 'r',
								align: 't',
	...
	<style type="text/css">
		...
		body {
			...
			background-image: url(<?= base_url("assets/images/snow.jpg") ?>);
...
<div id="login">
	...
	<div class="wrapper">
		...
		<div class="login sign-in-htm">
			<form class="container offset1 loginform" id="login_form">
					...
					<section class="content">
							<span class="input input--hideo">
								<input class="input__field input__field--hideo" type="text" id="login-username"
									   name='login_account' autocomplete="off" placeholder="Input your account" tabindex="1"
									   maxlength="15"/>
						...
						<span class="input input--hideo">
								<input class="input__field input__field--hideo" type="password" id="login-password"
									   name='login_password' placeholder="Input your password" tabindex="2" maxlength="15"/>
				...
				<div class="form-actions">
					...
					<input class="btn btn-primary" type="button" tabindex="3" onClick="login()" value="Login"
						   name="login_button" style="color:white;" />
				</div>
			</form>
		</div>
		<!-- 忘记密码页面 -->
		...
		<div class="login sign-up-htm">
			<form action="#" method="post" class="container offset1 loginform" id="register_form">
				<!-- 猫头鹰控件 -->
					...
					<section class="content">
							<span class="input input--hideo">
								<input class="input__field input__field--hideo" type="text" id="register-username"
									   name="register_account" autocomplete="off" placeholder="Input your account"
									   maxlength="15"/>
						...
						<span class="input input--hideo">
								<input class="input__field input__field--hideo" type="text" id="register-name"
									   name="register_name" placeholder="Input your name" maxlength="15"/>
						...
						<span class="input input--hideo">
								<input class="input__field input__field--hideo" type="password" id="register-password"
									   name="register_password" placeholder="Input your password" maxlength="15"/>
						...
						<span class="input input--hideo">
								<input class="input__field input__field--hideo" type="password" id="register-repassword"
									   name="register_confirm_password" placeholder="Input your password again"
									   maxlength="15"/>
